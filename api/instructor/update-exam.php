...
if (!$stmt->fetch()) {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'Not authorized to edit this exam']);
    exit;
}

$query = "UPDATE exams SET exam_title=:exam_title, course_id=:course_id, description=:description,
          exam_type=:exam_type, total_marks=:total_marks, passing_score=:passing_score,
          duration_minutes=:duration_minutes, available_from=:available_from,
          available_until=:available_until, randomize_questions=:randomize_questions,
          show_results=:show_results, attempts_allowed=:attempts_allowed, status=:status,
          exam_password=:exam_password WHERE id=:id";
